...
df1 = pd.read_sql_table("v_monthly_electric",engine)
elecusage_vals = df1['m_kwh_avg']
dater = df1['NORM_DATE']

# Make the figure wider to see things better
plt.figure(figsize=(15,10), dpi=92, edgecolor='#406054')

plt.plot_date(dater, elecusage_vals, linestyle="solid", color="#5a7d9a", linewidth=1, marker='', label="Monthly average electric use (kwh)")
...
plt.title("Monthly Avg Electrical Usage")
plt.grid('True')
plt.legend()
plt.tight_layout() # optional to fix certain layout issues.
plt.savefig('monthly_avg_electric.png')
plt.show()